...
# Изображение победы
images_list = os.listdir("images_mem")
winImagePrev, winImageList = [], []
for i in images_list:
    winImagePrev.append(pygame.image.load(f"images_mem/{i}"))
for i in winImagePrev:
    winImage = pygame.transform.scale(i, (WIDTH, HEIGHT))
    winImage.set_alpha(170)
    winImageList.append(winImage)

# Звуки
hitSound = pygame.mixer.Sound("sounds_mem/iamete/iamete.mp3")
hitSound2 = pygame.mixer.Sound("sounds_mem/iamete/gaci.mp3")

scoreSound, losingSound = [], []
for i in os.listdir("sounds_mem/score"):
    scoreSound.append(pygame.mixer.Sound(f"sounds_mem/score/{i}"))
for i in os.listdir("sounds_mem/losing"):
    losingSound.append(pygame.mixer.Sound(f"sounds_mem/losing/{i}"))
...
def main():
    ...
    while running:
        ...
        if pause == False:
            ...
            if pygame.Rect.colliderect(ball.getRect(), player1.getRect()):
                ball.hit()
                hitSound.play()
            if pygame.Rect.colliderect(ball.getRect(), player2.getRect()):
                ball.hit()
                hitSound2.play()
            ...
            if point:
                ...
                if player1Score == 7 or player2Score == 7:
                    ...
                    scoreSound[random.randint(0, len(scoreSound) - 2)].play()
            ...
            if imageFlag == True:
                image_number = random.randint(0, len(images_list) - 1 )
                imageFlag = False
            screen.blit(winImageList[image_number], (0, 0)) # image_number определяется до использования в основном if


            if losingSoundFlag == True:
                losingSound[random.randint(0, 1)].play()
                losingSoundFlag = False
            if player1Score == 7: #TODO
                menu(1)
            else:
                menu(2)
